...
func JoinGroup(hostp2p *nodehost.P2P, username string, groupname string) (*GroupRoom, error) {
...
func (jr *JoinRequest) handleJoinRequest(from peer.ID) {
	...
	if CurrentGroupShareKey.GroupName != jr.GroupName {
	...
	case "<y>":
		...
		str, err := MentorInfoObj.Host.NewStream(*MentorInfoObj.MentCTX, from, GroupJoinReplyProtocol)
		if err != nil {
